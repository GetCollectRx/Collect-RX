...
| P10-14 | **Confirm whether `platform_admin → 'group_admin'` in `signBriefSessionToken` is intentional** | Found during P10-09 but deliberately *not* changed: the JWT actually issued at login (`authToken.ts`'s `signBriefSessionToken`) embeds legacy `role: 'group_admin'` for a `platform_admin` persona login — never `'platform_dev'`, even though `respondPlatformUserLogin`'s JSON response and `/me` (post-P10-09-fix) both report `role: 'platform_dev'` for the same user. `authorizeRole('platform_dev')` gates at least one sensitive action (`POST /users/:userId/promote-owner`, comment: "Only platform_dev (Khalid) can promote a user to practice_owner"), which reads like a deliberate single-super-admin-vs-persona-based-admins design, not an oversight — but that's a guess. Confirm with whoever owns this RBAC design before touching it; if it's actually a bug, it means no `platform_admin` persona (as opposed to the shared-password platform-dev login) can ever pass an `authorizeRole('platform_dev')` gate, which would be worth knowing either way. |
