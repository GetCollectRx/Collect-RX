...
| P10-13 | **Wire `AuditorGrant` into practice-listing/authorization, not just role strings** | Found during P10-09: `isCrossPracticeReader()` (`accessControl/types.ts`) and `GET /api/auth/me`'s practices-list logic both key off `userRole === 'billing_ops_manager' \| platform_admin'` only — neither queries the `AuditorGrant` table at all. After the P10-09 fixes, an auditor can log in and stay authenticated, but `/api/auth/me` still returns `practices: []` even for an auditor with a global (`practiceId: null`) grant — confirmed live via `curl`. Figure out where `AuditorGrant` scoping is actually meant to apply (maybe it's enforced per-route elsewhere for specific auditor endpoints and the generic `/me` listing was never meant to use it — needs a design read, not just a code fix) and wire it in if it isn't. |
