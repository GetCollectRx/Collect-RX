...
export async function upsertBalances(rows: BalanceRow[], practiceId?: string): Promise<UpsertResults> {
  ...
  for (const row of rows) {
    try {
      ...
      const patientOwes = parseFloat(String(row.patient_owes || 0));

      if (!firstName || !lastName || isNaN(patientOwes) || patientOwes <= 0) {
        results.skipped++;
        continue;
      }
      ...
      console.error('Balance upsert error', { patient: row.abeldent_patient_id, error: (err as Error).message });
      results.errors.push({ patient: row.abeldent_patient_id || 'unknown', error: (err as Error).message });
    }
  }
