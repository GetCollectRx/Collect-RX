...
| 1.3 | Login JWT not in JSON body | **Pass** | HttpOnly `crx_access` cookie (`httpOnly`, `secure` in prod, `sameSite`) — see `authToken.ts` `cookieOptions()`. **Fixed (2026-07-28)**: the former `GET /api/auth/reset-password/token/:userId` manual relay (returned a password-reset token in JSON to `platform_dev`) is gone. Reset tokens are now hashed (SHA-256) at rest and never returned to any caller, including admins; `POST /api/auth/reset-password/resend/:userId` replaces it and only triggers a fresh reset email |
