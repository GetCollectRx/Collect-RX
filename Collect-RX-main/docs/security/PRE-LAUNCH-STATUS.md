...
| 2.3 | IDOR static audit test | **Pass, but coverage is stale** | `tests/idorPracticeScope.audit.test.ts` passes, but its `AUTH_ROUTE_FILES` allow-list predates the multi-tenant admin/front-desk/reports routers and does not include `platformPersonaAdminApi.ts`, `frontDeskApi.ts`, `practiceReportsApi.ts`, `groupAdminRoutes.ts`, or `complianceRoutes.ts`. Manually verified these are correctly session-scoped this session, but the automated regression guard doesn't cover them — a future change to any of those files could silently break scoping with no test catching it |
...
| 3.2 | Postgres RLS | **N/A** | App-level `practiceId` scoping, as before |
