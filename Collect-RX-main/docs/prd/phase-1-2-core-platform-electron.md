...
- `discover-schema.cjs` to map actual Abeldent table and column names (run once per AbelDent installation before sync goes live)
...
- Multi-practice SaaS (implemented: signup, per-tenant `Practice` rows, PostgreSQL RLS, session scoping)
