...
describe.skipIf(!dbReady)('Platform developer access control', () => {
  it('blocks PHI routes and allows redacted insurance claims', async () => {
  ...
  it('practice login still has phiAccess', async () => {
    const practice = await createPracticeForTests(prisma);
    const login = await request(app)
      .post('/api/auth/login')
      .send({ practiceId: practice.id, password: FIXTURE_PRACTICE_PASSWORD });
    expect(login.status).toBe(200);
    expect(login.body.role).toBe('practice');
