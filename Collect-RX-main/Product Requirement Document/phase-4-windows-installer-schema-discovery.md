...
**Owner:** Khalid  
**Dependencies:** Pilot-practice availability for an on-site session; Phase 1–2 complete
...
CollectRx cannot sync real claim data until the actual Abeldent database schema is confirmed on the pilot practice's machine. Abeldent Local Plus uses an on-premise SQL Server with a schema that may vary across installations. Additionally, there is no packaged `.exe` installer that a pilot practice can install — the product only exists as a development build. Both gaps must be closed before the pilot can start.

---

## Goals

- Run `discover-schema.cjs` against the pilot practice's Abeldent SQL Server to map actual table and column names
...
- Deliver an installable product the pilot practice can run without developer involvement
...
| Install time on pilot-practice machine | < 5 minutes |
...
### Schema Discovery
- Run `discover-schema.cjs` on the pilot practice's Windows machine against its Abeldent Local Plus SQL Server
...
### Sync Validation
- After schema discovery: run sync against the pilot practice's Abeldent and confirm patient records appear in dashboard
...
- Must be run on or remotely connected to the pilot practice's Windows machine
...
- [x] `discover-schema.cjs` script ships in repo (`npm run abeldent:discover`); run on the pilot practice's Windows PC against SQL Server
...
- [ ] Pilot-practice machine runs CollectRx from installer without developer assistance *(pilot)*
