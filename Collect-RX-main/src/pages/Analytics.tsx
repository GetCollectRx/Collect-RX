...
import {
  StatTile, Card, CardHeader, Badge,
    ...
    apiFetch(`/api/analytics/practice-performance?practiceId=${practiceId}`)
      .then(async (r) => {
        const data = await parseApiJson<{
          success?: boolean
          data?: typeof practicePerf
          error?: string
        }>(r)
        if (!r.ok) {
          throw new Error(data.error || 'Analytics unavailable')
        }
        setPracticePerf(data.data ?? null)
      })
      .catch((e) => setError((e as Error).message))
      .finally(() => setLoading(false))
  }, [practiceId])
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              Insurance claims recovery — dollars at risk, carrier outcomes, and time saved
            </p>
          </div>
          {practiceId && (
            <a
