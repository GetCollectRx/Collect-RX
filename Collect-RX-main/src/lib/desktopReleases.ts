...
export const DESKTOP_RELEASES_PAGE = 'https://github.com/GetCollectRx/Collect-RX/releases/latest'
...
const ASSET_PATTERNS: { platform: DesktopPlatform; label: string; test: (name: string) => boolean }[] = [
  ...
  { platform: 'windows', label: 'Windows', test: (n) => /\.exe$/i.test(n) },
...
export async function fetchDesktopReleases(): Promise<DesktopReleaseInfo | null> {
  try {
    const res = await fetch('/api/desktop/releases')
    if (!res.ok) return null
    const json = (await res.json()) as { success?: boolean; data?: DesktopReleaseInfo }
...
export function fallbackDesktopReleaseInfo(version = 'latest'): DesktopReleaseInfo {
  return {
    version,
    publishedAt: null,
    releasePageUrl: DESKTOP_RELEASES_PAGE,
    assets: [
      { platform: 'windows', label: 'Windows installer', fileName: 'CollectRx-Setup.exe', downloadUrl: DESKTOP_RELEASES_PAGE },
      { platform: 'mac-arm64', label: 'macOS (Apple Silicon)', fileName: 'CollectRx-arm64-mac.zip', downloadUrl: DESKTOP_RELEASES_PAGE },
      { platform: 'mac-x64', label: 'macOS (Intel)', fileName: 'CollectRx-mac.zip', downloadUrl: DESKTOP_RELEASES_PAGE },
      { platform: 'linux', label: 'Linux (AppImage)', fileName: 'CollectRx.AppImage', downloadUrl: DESKTOP_RELEASES_PAGE },
    ],
  }
}
