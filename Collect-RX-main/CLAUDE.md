...
A green pre-push hook proves the code you touched is internally correct. It does not prove the change belongs, or that it leaves everything *around* it working. Both of those are on you, every push, no exceptions:

1. **Confirm the change matches current product direction before writing code, not after.** Check `docs/operations/PATH-TO-DELIVERY.md` (the live status source — see the repo-root `CLAUDE.md`'s doc-authority order) and any relevant ADR in `docs/adr/` for what's actually in scope right now. A technically-correct feature built against a stale assumption (an old doc, a deprecated flow, a retired campaign push) is still the wrong thing to ship. If the direction is ambiguous or the work is a meaningful architectural choice, say so and ask — don't guess and push.
2. **Before pushing, map what else touches the code you changed — both directions.** What does your change depend on (a package version, a config value, another service's behavior) that could shift under you? What depends on your change (other routes, other tests, other docs, a different carrier/vendor path) that your diff might silently break? `grep`/search for other callers and other tests of anything you modified, not just the tests you wrote for your own change. Passing your own new tests is necessary, not sufficient.
3. **A cross-cutting dependency bump (npm package, shared util, shared config) is never "just the one file you meant to touch."** Trace who else imports it before you push, and re-run the tests for those callers, not only the ones you were already working on.

This isn't hypothetical: past sessions in this repo have found that a required dependency fix (an `npm audit` bump) needed verifying against every module that called the bumped package's API, and that a test failure that looked like a flaky timing issue was actually a real, deterministic product rule (a carrier-specific dispatch gate) with its own separate rationale documented elsewhere in the codebase — fixing the *symptom* (bump the timeout) would have shipped a test that no longer meant anything. Both took deliberately looking past the first plausible explanation before pushing. Do that same check by default, not only when something looks suspicious.
