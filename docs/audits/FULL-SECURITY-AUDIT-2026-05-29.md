...
0b. **Hash password-reset tokens at rest.** **Fixed (2026-07-28).** Tokens are now SHA-256-hashed before storage and looked up by hash (`authRoutes.ts`, `hashResetToken()`). The admin-relay endpoint that used to return a plaintext token to `platform_dev` (`GET /reset-password/token/:userId`) is gone; it's replaced by `POST /reset-password/resend/:userId`, which invalidates the old token and sends a fresh reset email — it never returns a token to anyone, matching how Google/Meta-style account recovery works (support can trigger a resend, never view the secret). Still open: raising the password minimum above 8 and a breached-password check were not part of this pass.
