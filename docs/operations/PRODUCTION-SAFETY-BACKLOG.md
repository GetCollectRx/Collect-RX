...
- **Graceful shutdown for the API process** (this was P0.1 in the original backlog, narrowed to what matters for this week's deploys): `SIGTERM`/`SIGINT` now drive a phased shutdown — `/api/health/ready` flips to 503 immediately (`__setShuttingDownForTests` test seam), `/ws/desk` clients get a real 1001 close frame (`closeAllDeskConnections()` in `deskWs.ts`), the desk queue tick is allowed to finish in flight rather than being abandoned mid-dispatch (`drainDeskQueueEngine()` in `queueEngine.ts`), then the HTTP server drains and Prisma disconnects — all bounded by `GRACEFUL_SHUTDOWN_JOB_TIMEOUT_MS`/`GRACEFUL_SHUTDOWN_TOTAL_TIMEOUT_MS` with a force-exit fallback. Replaces the previous bare `SIGTERM`-only handler that called `prisma.$disconnect()` with no drain at all.
- Tests: `tests/gracefulShutdown.test.ts`, `tests/deskWsShutdown.test.ts`, `tests/healthReadyShutdown.test.ts`, `tests/opsAlertingConfigCheck.test.ts`, `tests/registerSchedulersRetry.test.ts` — 40 new tests, all passing; full existing suite still green (2 pre-existing unrelated failures in this sandbox from missing `DATABASE_URL`/`VAPI_WEBHOOK_SECRET`, confirmed present without these changes too).

**Deliberately not done in this pass, still fully backlog:** the 531-call-site logging consolidation (P0.3), Vapi circuit breaker (P0.2), BullMQ dead-letter queue + admin endpoints (P0.4's DLQ half — retry landed, DLQ did not), deep `/api/diagnostics` (P1.2), Prometheus metrics work (P1.3), runbooks (P2.1), deploy/rollback rehearsal (P2.2), and all of P3. Rushing the logging migration in particular was explicitly rejected — see rationale in this workstream's chat history.
...
### P0.1 — Graceful shutdown (API process)
**Real gap:** `src/server/index.ts:671-675` — SIGTERM handler calls `prisma.$disconnect()` and exits with no `server.close()`, no HTTP drain, no WebSocket close, no desk-queue-tick drain. No SIGINT handler at all (dev `Ctrl+C` bypasses cleanup entirely).
**Not a gap:** `src/server/workerEntry.ts` already does phased shutdown correctly (health server → BullMQ worker → Prisma → Redis, on both signals) — use it as the reference pattern rather than reinventing one.
**Build:**
- Add `SIGINT` alongside `SIGTERM` in `index.ts`.
- Phased shutdown: stop `server.listen()` accepting new connections (`server.close()`) → close `/ws/desk` connections with code 1001 (`deskWs.ts` needs a `closeAll()` export) → let `runDeskQueueTick`'s `isTickRunning` guard finish its current tick, don't start a new one → `prisma.$disconnect()` → exit.
- Configurable timeouts (`GRACEFUL_SHUTDOWN_JOB_TIMEOUT_MS`, `GRACEFUL_SHUTDOWN_TOTAL_TIMEOUT_MS`) with force-exit + structured log on overrun.
- `/api/health/ready` must return 503 the instant shutdown begins (Fly should stop routing before the drain completes).
**Out of scope here:** the BullMQ worker process — already handled.

### P0.2 — Circuit breaker for Vapi client
**Real gap:** confirmed accurate. `src/vapi/client.ts:216-231` is a single timed `fetch`, no failure tracking, no open/half-open state.
**Build:** wrap outbound calls in `src/vapi/client.ts` with a small CLOSED/OPEN/HALF_OPEN breaker (new module, e.g. `src/vapi/circuitBreaker.ts`). Integrate the breaker's state into `src/server/frontDesk/queueEngine.ts`'s dispatch decision so an OPEN breaker causes a logged deferral, not a hung tick. Surface breaker state on `/api/health/metrics`. Keep the existing `isTickRunning` guard and slot-budget logic untouched — additive only.
**Safety note:** any change to dispatch decisions in `queueEngine.ts` must not bypass the CARRIER_BLOCK check that already gates dispatch — verify order of checks doesn't change.
