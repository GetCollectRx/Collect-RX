...
- **Structured logging consolidation — P0.3, foundation done + 6 of the highest-value files migrated; call-site migration is genuinely NOT complete across all 531 sites.** Be precise about what "done" means here before picking this up again:
  ...
  - **Call sites actually migrated (console.* → logger.*):** `src/server/workerEntry.ts`, `src/server/rulesEngine.ts`, `src/server/routes/authRoutes.ts` (this one also fixed a real PHI leak in passing — a raw email address was being interpolated directly into a log message string; it now goes through `safeMeta()`'s redaction like everything else), `src/routes/insurance.ts`, `src/webhooks/vapi.ts`, `src/server/index.ts` — 6 of the 7 files the original doc flagged as highest-count. Net effect on this repo's own lint gate: warning count dropped from 205 to **175** (console.log/debug calls, which the PRD rule treats as warnings, mostly disappeared behind the logger's two remaining internal `console.log`/`console.debug` calls).
  ...
  - **Still fully backlog:** the other ~111 files / ~470-ish remaining call sites (531 total minus what's listed above). No ESLint rule was added to ban new `console.*` in server code — this repo's existing `no-console` rule already treats `console.log`/`console.debug` as warnings (not errors) by deliberate PRD-gate policy (see `Collect-RX-main/CLAUDE.md`), and `console.warn`/`console.error` are explicitly sanctioned there. The original doc's ask for a hard *zero-console* CI-enforced rule conflicts with that established, deliberate policy — flagging the conflict rather than silently overriding this repo's own documented standard. Whoever continues this item should decide whether to keep console.warn/error as an accepted fallback for lower-value files, or extend the migration further; either is defensible, but it's a real decision, not a mechanical one.
  - Tests: `tests/observability-logger.test.ts` (redaction — including a specific test proving carrier names like "Sun Life" are NOT over-redacted — method-style API, correlation-ID auto-injection), `tests/correlationIdMiddleware.test.ts`, plus `resolveJobCorrelationId` coverage added to `tests/opsAlertingConfigCheck.test.ts`. 66 new tests, all passing.

**Deliberately not done in this pass, still fully backlog:** the remaining ~470 console-call-site migrations from P0.3, desk-queue-tick-level retry/backoff (P0.5), deep `/api/diagnostics` (P1.2), Prometheus-format metrics work if that's ever wanted instead of JSON (P1.3), runbooks (P2.1), deploy/rollback rehearsal (P2.2), and the rest of P3.
...
### P0.3 — Structured logging consolidation — 🟡 FOUNDATION DONE, MIGRATION PARTIAL
Foundation (logger consolidation, hardened redaction, correlation-ID propagation for HTTP + BullMQ) is fully implemented and tested — see "Shipped ahead of schedule" at the top of this doc. 6 of the 7 originally-flagged heaviest files are migrated (`index.ts`, `webhooks/vapi.ts`, `authRoutes.ts`, `insurance.ts`, `workerEntry.ts`, `rulesEngine.ts`); `auditAgent.ts` was deliberately excluded (CLI report printer, not server logging — converting it would break its purpose). Redaction was verified against real field names from `VapiCallParams` rather than assumed. The ESLint "ban console" rule was **not** added — it would conflict with this repo's own deliberate `no-console` policy (console.warn/error sanctioned, console.log/debug already warnings-only) documented in `Collect-RX-main/CLAUDE.md`; flagged as a real decision for whoever continues this, not something to silently override.
**Still open:** ~111 files / ~470 call sites remain unmigrated. Pick up by grepping `console\.` across `src/`, skipping any file that turns out to be a CLI/script entry point (check for a `process.argv[1]` main-module guard like `auditAgent.ts` and `index.ts` before assuming it's server-request-path logging).
