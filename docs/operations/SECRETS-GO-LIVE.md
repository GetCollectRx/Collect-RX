...
1. **Where they live** — your host’s secret store, AWS Parameter Store, or the “Variables” UI. Never in `.env` committed to git, screenshots in Slack, or public tickets.
...
4. **Break-glass** — A second admin path to the host (e.g. the hosting platform’s team login with MFA) for when primary operators are locked out. Document who has access; audit when used.
