...
3. **Rotation** — Generate new value in provider → set new var in host → **deploy** or restart so all instances read the new value → revoke the old value in the provider. For Stripe/SendGrid, prefer creating a **new** key, switching traffic, then deleting the old one.
