...
- [ ] P4-04 Stripe Billing (practice SaaS) — **code complete + fail-closed (2026-07-17):** catalog = core/growth/scale from `tiers.ts`; `/billing` tier picker → Checkout → Portal; webhook syncs `practice.billingTier` from the price ID; call gate blocks paid tiers with missing/mismatched Stripe config under `SUBSCRIPTION_ENFORCE=1`. **Blocked on secrets only:** Stripe **test** `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` + `STRIPE_PRICE_CORE|GROWTH|SCALE`; then webhook → `https://collect-rx-staging.fly.dev/api/stripe/webhook` and the verification flow in [PHASE4-GO-LIVE.md](PHASE4-GO-LIVE.md) §P4-04.
