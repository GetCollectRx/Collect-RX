/**
 * Analytics API routes — ClickHouse-backed
 *
 * POST /api/analytics/events      Ingest a batch of events from the SDK
 * GET  /api/analytics/summary     Headline numbers
 * GET  /api/analytics/pages       Time-on-page per route
 * GET  /api/analytics/clicks      Top clicked elements (optionally ?path=)
 * GET  /api/analytics/funnels     Funnel step completion + drop-off (?name=)
 * GET  /api/analytics/sessions    Recent session path traces (?limit=)
 *
...
import { safeInsert, safeQuery } from '../analytics/clickhouse';
