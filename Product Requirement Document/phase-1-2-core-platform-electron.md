...
- `discover-schema.js` to map actual Abeldent table and column names (must run on Dr. Hasan's machine before sync goes live)
...
- `discover-schema.js` must be run on Dr. Hasan's Abeldent instance before production sync
